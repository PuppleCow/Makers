...
import kotlinx.android.synthetic.main.fragment_setting_resetting_password2.*
import com.pupplecow.myapplication.ui.settings.SettingMyInformationFragment

class SettingResettingPassword2Fragment:Fragment() {

    companion object{
        fun newInstance(): SettingResettingPassword2Fragment{
            return SettingResettingPassword2Fragment()
        }
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        val view=inflater.inflate(R.layout.fragment_setting_resetting_password2,container,false)
        return view
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        // 다음버튼 눌렀을때
        resetting_password2_button1.setOnClickListener {
            //비밀번호 서로 같은지 확인하기
            val password = resetting_password2_password.text.toString()
            val passwordCheck = resetting_password2_password2.text.toString()
...
//            val b_passwordCheck:Boolean=Pattern.matches(s,passwordCheck)
            ...
            if (resetting_password2_password.length() != 6 || resetting_password2_password2.length() != 6) {
                resetting_password2_textView4.text = "글자 수가 맞지 않습니다. 다시 입력해주세요."
                resetting_password2_textView5.text = ""
            }else if(isNumOnly(password)==false || isNumOnly(passwordCheck)==false){
                // 숫자로만 이루어져있는 지 확인
                resetting_password2_textView4.text = "숫자로만 입력해주세요."
                resetting_password2_textView5.text = ""
            }
            else if (password != passwordCheck) {
                // 비밀번호 다를 때

                resetting_password2_textView4.text = "비밀번호가 맞지 않습니다. 다시 입력해주세요."
                resetting_password2_textView5.text = ""
            } else {
...
//                val intent = Intent(requireContext(), SettingMyInformationFragment::class.java)
